...
public class MyDeque<E> {
  ...
  private void resize() {
    E[] bigger = (E[]) new Object[data.length * 2 + 1]; //Let's just assume resize increases the maximum size by 1.
    for (int i = 0; i < size; i = i + 1) { //Copy over elements.
      bigger[i] = data[(start + i) % data.length];
    }
    data = bigger;
    start = 0;
    end = size - 1;
